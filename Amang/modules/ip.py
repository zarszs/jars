...
@ubot.on_message(filters.command("ip", cmd) & filters.me)
...
@ubot.on_message(filters.command("ipd", cmd) & filters.me)
...
__MOD__ = "ipsearch"
__HELP__ = f"""
Document for IP Search

• Command: <code>{cmd[0]}ip</code> [ip host]
• Function: Untuk mencari lokasi ip addres.

๏• Command: <code>{cmd[0]}ipd</code> [ip domain]
• Function: Untuk mencari lokasi ip domain.
"""